...
import com.spotzee.android.network.NotificationContentDeserializer
...
@Parcelize
data class SpotzeeNotification(
    val id: Long,
...
class SpotzeeAction(
    ...
    companion object {
        ...
        fun from(config: JSONObject?): SpotzeeAction? = config?.let { SpotzeeAction(it) }

        fun actionOpenUrl(url: String?): SpotzeeAction? =
            url?.let {
                val config = JSONObject()
                config.put("type", "openUrl")
                config.put("data", url)
                SpotzeeAction(config)
            }

        fun actionCustomAction(customActionName: String): SpotzeeAction? {
            val config = JSONObject()
            config.put("type", customActionName)
            return SpotzeeAction(config)
        }
    }
}
