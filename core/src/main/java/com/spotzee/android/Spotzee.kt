...
import com.spotzee.android.network.NetworkManager
...
open class Spotzee protected constructor(
     ...
     * Call identify whenever user traits (attributes) change to make sure they are updated.
     *
     * @param identity An object representing a Spotzee user identity
     */
     ...
     */
    suspend fun getNotifications(): Result<Page<SpotzeeNotification>> =
        network.get<Page<SpotzeeNotification>>(
            path = "notifications" ,
            user = Alias(
                anonymousId = getOrAndOrSetAnonymousId(),
                externalId = externalId
        ...
        withContext(Dispatchers.Main) {
            ...
            InAppDialogFragment.newInstance(
                ...
                delegate = object : InAppDelegate {
                    ...
                    override fun onNotificationShown(notification: SpotzeeNotification) {
     ...
     *
     * To allow for click tracking, all emails are click-wrapped in a Spotzee url
     * that then needs to be unwrapped for navigation purposes. This method
     * checks to see if a given URL is a Spotzee URL and if so, unwraps the url,
     * triggers the unwrapped URL and calls the Spotzee API to register that the
     * URL was executed.
     *
    ...
    fun handle(universalLink: Uri): Boolean {
        if (!isSpotzeeDeepLink(universalLink)) {
            return false
        }
    ...
    fun getUriRedirect(universalLink: Uri): Uri? {
        if (!isSpotzeeDeepLink(universalLink)) return null
    ...
    companion object {
         ...
         *
         */
        fun initialize(app: Application, config: Config): Spotzee = Spotzee(app, config)

        fun isSpotzeePush(extras: Bundle?): Boolean =
            extras?.getBoolean(Constants.SPOTZEE_KEY) == true || extras?.getString(Constants.SPOTZEE_KEY).toBoolean()

        fun isCheckMessagePush(extras: Bundle?): Boolean =
            isSpotzeePush(extras) &&
